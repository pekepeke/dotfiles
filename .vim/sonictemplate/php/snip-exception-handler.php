...
function php_error_handler($errno, $errmsg, $errfile, $errline, $errcontext) {
	...
	fputs(STDERR, "PHP " . self::_format_error($errno, $errmsg, $errfile, $errline, $errcontext). "\n");
	return false;
}
...
function php_shutdown_handler() {
	...
	if ($error['type'] == E_ERROR) {
		fputs(STDERR, sprintf("Fatal Error : %s - %s[%d]", $error['message'], $error['file'], $error['line']) ."\n");
		exit(255);
	}
	return false;
}
...
function php_exception_handler($e) {
	fputs(STDERR, sprintf("Uncaught Exception : %s - %s[%d](%d)", $e->getMessage(), $e->getFile(), $e->getLine(), $e->getCode()) . "\n");
	exit(255);
	return false;
}
