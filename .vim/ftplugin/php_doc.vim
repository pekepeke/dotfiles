...
let g:pdv_cfg_CommentHead = get(g:, 'pdv_cfg_CommentHead', "/**")
let g:pdv_cfg_Comment1 = get(g:, 'pdv_cfg_Comment1', " * ")
let g:pdv_cfg_Commentn = get(g:, 'pdv_cfg_Commentn', " * ")
let g:pdv_cfg_CommentTail = get(g:, 'pdv_cfg_CommentTail', " */")
let g:pdv_cfg_CommentEnd = get(g:, 'pdv_cfg_CommentEnd', "/* }}" . "} */")

let g:pdv_cfg_EnalbedFoldMarker = get(g:, 'pdv_cfg_EnalbedFoldMarker', 1)
let g:pdv_cfg_CommentStart = get(g:, 'pdv_cfg_CommentStart', "// ")
let g:pdv_cfg_CommentStartTail = get(g:, 'pdv_cfg_CommentStartTail', "")
let g:pdv_cfg_CommentSingle = get(g:, 'pdv_cfg_CommentSingle', "//")

" Default values
let g:pdv_cfg_Type = get(g:, 'pdv_cfg_Type', "mixed")
" let g:pdv_cfg_Package = "Framework"
let g:pdv_cfg_Package = get(g:, 'pdv_cfg_Package', "Webdav")
let g:pdv_cfg_Version = get(g:, 'pdv_cfg_Version', "//autogen//")
let g:pdv_cfg_Author = get(g:, 'pdv_cfg_Author', "")
...
let g:pdv_cfg_ReturnVal = get(g:, 'pdv_cfg_ReturnVal', "void")
...
let g:pdv_cfg_php4guessval = get(g:, 'pdv_cfg_php4guessval', "protected")

"
" Regular expressions
"

let g:pdv_re_comment = get(g:, 'pdv_re_comment', ' *\*/ *')

" (private|protected|public)
let g:pdv_re_scope = get(g:, 'pdv_re_scope', '\(private\|protected\|public\)')
" (static)
let g:pdv_re_static = get(g:, 'pdv_re_static', '\(static\)')
" (abstract)
let g:pdv_re_abstract = get(g:, 'pdv_re_abstract', '\(abstract\)')
" (final)
let g:pdv_re_final = get(g:, 'pdv_re_final', '\(final\)')

" [:space:]*(private|protected|public|static|abstract)*[:space:]+[:identifier:]+\([:params:]\)
let g:pdv_re_func = get(g:, 'pdv_re_func', '^\s*\([a-zA-Z ]*\)function\s\+\([^ (]\+\)\s*(\s*\(.*\)\s*)\s*[{;]\?$')
let g:pdv_re_funcend = get(g:, 'pdv_re_funcend', '^\s*}$')
" [:typehint:]*[:space:]*$[:identifier]\([:space:]*=[:space:]*[:value:]\)?
let g:pdv_re_param = get(g:, 'pdv_re_param', ' *\([^ &]*\) *&\?\$\([A-Za-z_][A-Za-z0-9_]*\) *=\? *\(.*\)\?$')

" [:space:]*(private|protected|public\)[:space:]*$[:identifier:]+\([:space:]*=[:space:]*[:value:]+\)*;
let g:pdv_re_attribute = get(g:, 'pdv_re_attribute', '^\s*\(\(private\|public\|protected\|var\|static\)\+\)\s*\$\([^ ;=]\+\)[ =]*\(.*\);\?$')

" [:spacce:]*(abstract|final|)[:space:]*(class|interface)+[:space:]+\(extends ([:identifier:])\)?[:space:]*\(implements ([:identifier:][, ]*)+\)?
let g:pdv_re_class = get(g:, 'pdv_re_class', '^\s*\([a-zA-Z]*\)\s*\(interface\|class\)\s*\([^ ]\+\)\s*\(extends\)\?\s*\([a-zA-Z0-9]*\)\?\s*\(implements*\)\? *\([a-zA-Z0-9_ ,]*\)\?.*$')

let g:pdv_re_array = get(g:, 'pdv_re_array', "^array *(.*")
let g:pdv_re_float = get(g:, 'pdv_re_float', '^[0-9.]\+')
let g:pdv_re_int = get(g:, 'pdv_re_int', '^[0-9]\+$')
let g:pdv_re_string = get(g:, 'pdv_re_string', "['\"].*")
let g:pdv_re_bool = get(g:, 'pdv_re_bool', "[true false]")

let g:pdv_re_indent = get(g:, 'pdv_re_indent', '^\s*')

" Shortcuts for editing the text:
let g:pdv_cfg_BOL = get(g:, 'pdv_cfg_BOL', "norm! o")
let g:pdv_cfg_EOL = get(g:, 'pdv_cfg_EOL', "")
...
func! PhpDocFunc()
    ...
    if get(b:, 'pdv_cfg_EnalbedFoldMarker', g:pdv_cfg_EnalbedFoldMarker)
      exec l:txtBOL . g:pdv_cfg_CommentStart . l:scope ." "
            \ . funcname . "(" . l:params . ") {{" . "{"
            \ . g:pdv_cfg_CommentStartTail . g:pdv_cfg_EOL
    endif
...
func! PhpDocClass()
	...
	exe l:txtBOL . g:pdv_cfg_Commentn . "@copyright " . get(b:, 'pdv_cfg_Copyright', g:pdv_cfg_Copyright) . g:pdv_cfg_EOL
	exe l:txtBOL . g:pdv_cfg_Commentn . "@author " . get(b:, 'pdv_cfg_Author', g:pdv_cfg_Copyright) . g:pdv_cfg_EOL
	exe l:txtBOL . g:pdv_cfg_Commentn . "@license " . get(b:, 'pdv_cfg_License', g:pdv_cfg_License) . g:pdv_cfg_EOL
