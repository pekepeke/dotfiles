" A ref source for HTML5 documentation.
" Version: 0.0.1
" Author : hikaruworld
" License: MIT License
...
" config. {{{1
if !exists('g:ref_html5_path') " {{{2
  let g:ref_html5_path = ''
endif

if !exists('g:ref_html5_cmd') " {{{2
  let g:ref_html5_cmd =
...
let s:source = {'name': 'html5'} " {{{1

function! s:source.available() " {{{2
  return isdirectory(g:ref_html5_path) &&
  \ len(g:ref_html5_cmd)
endfunction
...
" functions. {{{1
function! s:syntax() " {{{2
  if exists('b:current_syntax') && b:current_syntax == 'ref-html5'
    return
  endif

  syntax clear

  let b:current_syntax = 'ref-html5'
endfunction



function! s:execute(file) "{{{2
  if type(g:ref_html5_cmd) == type('')
    let cmd = split(g:ref_html5_cmd, '\s\+')
  elseif type(g:ref_html5_cmd) == type([])
    let cmd = copy(g:ref_html5_cmd)
  else
    return ''
  endif
...
function! s:gather_func(name) "{{{2
  let list = glob(g:ref_html5_path . '/*')
  return map(split(list, "\n"),
        \ 'substitute(v:val, "\\v".g:ref_html5_path."/", "", "")')
endfunction
...
function! s:cache() " {{{2
  return ref#cache('html5', 'function', s:func('gather_func'))
endfunction



function! ref#html5#define() " {{{2
  return s:source
endfunction
call ref#register_detection('html', 'html5', 'append')
call ref#register_detection('xhtml', 'html5', 'append')
