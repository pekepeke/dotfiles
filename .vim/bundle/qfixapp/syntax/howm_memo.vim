...
"syntax match txtFile display '\[:\?\zs\([A-Za-z]:[/\\]\|\~\/\)[^:]\+\ze:\(\(title\|image\)\(=[^\]]*\)\?\)\?]'
"syntax match txtUrl display  '\[:&\?\zs\(howm\|rel\|https\|http\|ftp\|file\)://[^:]\+\ze:[^\]]*]'
syntax match txtFile display '\[:\?&\?\zs\([A-Za-z]:[/\\]\|\~\/\)[^:]\+\ze:[^\]]*]'
syntax match txtUrl display '\[:\?&\?\zs\(howm\|rel\|https\|http\|ftp\|file\)://[^:]\+\ze:[^\]]*]'
...
if exists('g:QFixHowm_Title') && exists('g:QFixHowm_EscapeTitle')
  ...
  "exe 'syn region howmTitle start="^'.s:QFixHowm_Title.'" end="$" contains=CONTAINED'
