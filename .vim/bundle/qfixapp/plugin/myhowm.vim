...
if !exists('g:QFixHowm_SearchDiaryFile')
  let g:QFixHowm_SearchDiaryFile = '**/\(^\|[/\\]\)[1-2]*-000000.*'
endif
"検索時にカーソル位置の単語を拾う
...
if g:QFixHowm_Wiki == 2
  let g:QFixHowm_FileType = g:QFixHowm_FileType.'.vimwiki'
endif
...
if g:QFixHowm_Default_Key > 0
  ...
  if s:debug
    exec 'silent! nnoremap <unique> <silent> '.s:QFixHowm_Key.'A :call QFixHowmListDiary()<CR>'
  endif
...
augroup QFixHowmMRU
  ...
  if s:debug
    exec "autocmd BufRead,BufNewFile,BufEnter * call QFixHowmMRUBufEnter()"
    exec "autocmd BufLeave    * call QFixHowmMRUBufLeave()"
    exec "autocmd CursorMoved * call QFixHowmMRUCursorMoved()"
  else
...
function! QFixHowmOpenStr(str)
  call s:openstr(a:str)
endfunction
...
function! QFixHowmSaveMru(write)
  ...
  if ext !~ g:QFixHowm_FileExt
    if !exists('g:QFixHowm_MRUExt') || ext !~ g:QFixHowm_MRUExt
      return
    endif
  endif
  let file = expand('%:p')
...
"        let text = getline(sline)
  ...
  if text == ''
    ...
    for l in range(1,line('$'))
      let ltext = getline(l)
      if ltext != '' && ltext !~ pattern
        let text = ltext
        let idx = 0
        for d in s:MruDic
          if d['filename'] == file && d['text'] !~ '^'.l:QFixHowm_Title
...
function! QFixHowmFileLink()
  let file = expand("%:p")
  let file = substitute(file, '\\', '/', 'g')
  let hdir = fnamemodify(g:howm_dir, ":p")
...
function! QFixHowmPairFile()
  if expand('%:e') =~ g:QFixHowm_FileExt
    return
  endif
