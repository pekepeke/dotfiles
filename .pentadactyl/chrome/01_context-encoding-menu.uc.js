...
init: {
  let encodingMenu = document.getElementById("charsetMenu").cloneNode(true);
  encodingMenu.id = "context-encoding-menu";
  encodingMenu.removeAttribute("oncommand");
  encodingMenu.addEventListener("command", onEncodingMenuCommand, false);
  document.getElementById("contentAreaContextMenu").insertBefore(
    encodingMenu, document.getElementById("context-sep-properties"));
...
function setCharset(aCharset) {
  var webNav = document.commandDispatcher.focusedWindow
                       .QueryInterface(Ci.nsIInterfaceRequestor)
                       .getInterface(nsIWebNavigation);
  webNav.QueryInterface(Ci.nsIDocCharset).charset = aCharset;
  reloadWithCharsetChange(webNav);
}

function reloadWithCharsetChange(aWebNav) {
  try {
    aWebNav = aWebNav.sessionHistory.QueryInterface(nsIWebNavigation);
  } catch (e) {
  }
...
function updateEncodingMenus() {
  ...
  updateEncodingMenu(document.getElementById("charsetMenu"), content, detector);
  updateEncodingMenu(document.getElementById("context-encoding-menu"),
                     document.commandDispatcher.focusedWindow, detector);
}
