...
const gTimeoutName = "ldr_connection_timeout";
...
function checkCount(id){
	...
	try {
		...
		var timerId = setTimeout(function(xhr) { xhr.abort(); liberator.echoerr("LDR Unread Counter: connection closed"); }, manager.timeout, xhr);
		xhr.send(null);
...
var manager = {
	...
	get timeout() {
		var timeout = liberator.globalVariables[gTimeoutName];
		if (timeout) {
			timeout = parseInt(timeout, 10);
		}
		return (isNaN(timeout) ? 1000 : timeout);
	},
	get count(){
		return unreadCount;
	},
