...
#
# Version 8.3.15
# - perlValue now prints NSCFDate as a unix timestamp using timeIntervalSince1970().
#   Contributed by Adam Reed <adam.reed@ (remove) anu.edu.au>
#
# Version 8.3.14
...
###########################################################################
#
# To use this file on Mac OS X place it in /usr/local/lib/
#
# In scripts use this code.
#
# use Foundation;
...
###########################################################################
#
# Instructions last updated 2011, June 7th (8.3.13)
#
...
sub pathsThatMatchPartialPathLooper {
	...
	if ( $object and $$object ) {
		if ( $object->isKindOfClass_( NSArray->class ) ) {
			...
			while ( my $keyCocoa = $enumerator->nextObject() and $$keyCocoa ) {
				# compare
			...
			if ( $last_container_type eq $perlplist_array_token or $last_container_type eq $perlplist_array_insert_token ) {
				if ( $parentContainer->isKindOfClass_( NSArray->class ) ) {
					if ( $value_token eq "add" or $value_token eq "+" or $value_token eq "push" ) {
						...
						my $present = $parentContainer->containsObject_( $addMe );
...
sub diffLoopThroughNSArrays {
	...
	# Find missing
	for ( my $ii = $counter; $ii < $cocoaArray2->count; $ii++ ) {
...
sub objectType {
	...
	if ( $value =~ /^(__)?NS(CF)?Array/ ) {
		return "NSCFArray";
	} elsif ( $value =~ /^(__)?NS(CF)?Dictionary/ ) {
		return "NSCFDictionary";
	} elsif ( $value =~ /^(__)?NS(CF)?(Constant)?String/ ) {
		return "NSCFString";
	} elsif ( $value =~ /^(__)?NS(CF)?Boolean/ ) {
		return "NSCFBoolean";
	} elsif ( $value =~ /^(__)?NS(CF)?Number/ ) {
		return "NSCFNumber";
	} elsif ( $value =~ /^(__)?NS(CF)?(Tagged)?Date/ ) {
		return "NSCFDate";
	} elsif ( $value =~ /^(__)?NS(CF)?(Concrete)?Data/ ) {
		return "NSCFData";
	} else {
...
sub perlValue {
	...
	if ( objectType ( $object ) eq "NSCFArray" ) {
		return perlArrayFromNSArray( $object, 1 );
	} elsif ( objectType ( $object ) eq "NSCFDictionary" ) {
		return perlHashFromNSDict( $object, 1 );
	} elsif ( objectType ( $object ) eq "NSCFData" ) {
		my $hex = $object->description()->UTF8String();
		return hexToBase64($hex);
	} elsif ( objectType ( $object ) eq "NSCFDate" ) {
		# Unix timestamp
		return $object->timeIntervalSince1970();
		# - Formatted to the local timezone
		# return $object->descriptionWithLocale_(NSLocale->currentLocale())->UTF8String();
	} else {
